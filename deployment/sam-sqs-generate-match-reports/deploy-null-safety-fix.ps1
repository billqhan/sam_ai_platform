# PowerShell script to deploy the Null Safety fix
Write-Host "Deploying Null Safety Fix..." -ForegroundColor Green

$FUNCTION_NAME = "ktest-sam-sqs-generate-match-reports-dev"
$REGION = "us-east-1"
$PACKAGE_NAME = "lambda-deployment-package-v9-null-safety-fix.zip"
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "Function: $FUNCTION_NAME" -ForegroundColor Gray
    Write-Host "Region: $REGION" -ForegroundColor Gray
    Write-Host "Fix: Null safety for placeOfPerformance field" -ForegroundColor Gray
    
    Write-Host "`nKey Improvements:" -ForegroundColor Yellow
    Write-Host "- Fixed AttributeError when placeOfPerformance is None" -ForegroundColor Gray
    Write-Host "- Added proper null checks for nested dictionary access" -ForegroundColor Gray
    Write-Host "- Prevents crashes on opportunities with missing location data" -ForegroundColor Gray
    
    Write-Host "`nTest the fix with:" -ForegroundColor Yellow
    Write-Host "aws s3 cp s3://ktest-sam-extracted-json-resources-dev/2024-11-19/HR001125S0001.json s3://ktest-sam-extracted-json-resources-dev/HR001125S0001/HR001125S0001.json" -ForegroundColor Gray
