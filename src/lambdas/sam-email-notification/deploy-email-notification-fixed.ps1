...
.EXAMPLE
    .\deploy-email-notification-fixed.ps1 -Environment "dev" -BucketPrefix "ktest" -FromEmail "noreply@company.com" -SubscribersBucket "company-config"
#>
...
Write-Host ""
Write-Host "1. Deploying Lambda Function..." -ForegroundColor Cyan
try {
    $DeployScript = Join-Path $PSScriptRoot "..\..\infrastructure\scripts\update-lambda-code.ps1"
...
Write-Host ""
Write-Host "2. Configuring Environment Variables..." -ForegroundColor Cyan
$EnvVars = @{
...
Write-Host ""
Write-Host "3. Adding S3 Permission for Lambda..." -ForegroundColor Cyan
...
Write-Host ""
Write-Host "4. Configuring S3 Event Notification..." -ForegroundColor Cyan
...
Write-Host ""
Write-Host "5. Checking SES Email Verification..." -ForegroundColor Cyan
try {
    $QueryString = "VerificationAttributes.'$FromEmail'.VerificationStatus"
    $VerificationStatus = aws ses get-identity-verification-attributes --identities $FromEmail --region $Region --query $QueryString --output text 2>$null
...
Write-Host ""
Write-Host "6. Checking Subscribers File..." -ForegroundColor Cyan
try {
...
Write-Host ""
Write-Host "7. IAM Policy Requirements..." -ForegroundColor Cyan
