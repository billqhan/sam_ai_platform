...
def generate_response_template(json_data):
    '''Use Amazon Bedrock model with Knowledge Base to generate the response template'''
    
    # First, query the knowledge base for company capabilities
    opportunity_title = json_data.get('title', '')
    opportunity_description = json_data.get('description', '')
    
    # Create a query for the knowledge base
    kb_query = f"company capabilities relevant to {opportunity_title} {opportunity_description}"
    ...
    if kb_results:
        ...
        company_capabilities = "No specific company capabilities found in knowledge base."
    
    # Add static document references if needed
    static_references = """
...
"""
    company_capabilities += static_references
    
    # Construct the prompt for Bedrock model
...
**TO:**
{json_data.get('pointOfContact.fullName', 'Contracting Officer')}
{json_data.get('fullParentPathName', 'Government Agency')}
Email: {json_data.get('pointOfContact.email', 'contracting.officer@agency.gov')}
...
**SUBJECT:** Response to Solicitation {json_data.get('solicitationNumber', 'UNKNOWN')} - {json_data.get('title', 'Unknown Title')}

Dear {json_data.get('pointOfContact.fullName', 'Contracting Officer')},

[Your Company Name] respectfully submits this response to Solicitation {json_data.get('solicitationNumber', 'UNKNOWN')}, "{json_data.get('title', 'Unknown Title')}."
...
**Conclusion**
{json_data.get('rationale', 'We believe our qualifications and experience make us well-suited for this opportunity.')} We are committed to providing exceptional service and look forward to the opportunity to support your mission.
...
IMPORTANT: Replace ALL bracketed placeholders with actual company information. Do not leave any [Extract...] or [List...] instructions in the final output.'''

    try:
        logger.info(f"Using Bedrock model: {MODEL_ID}")
        # Call Bedrock model
        generated_text = call_bedrock_model(prompt_text)
        
        # Clean up AI thinking traces and meta-commentary
        cleaned_text = clean_ai_thinking_traces(generated_text)
        
        logger.info(f"Successfully generated response template using Bedrock model {MODEL_ID}")
        return cleaned_text
...
"""
    ...
    for line in response_content.splitlines():
        ...
        if line.startswith("**") and line.endswith("**") and len(line) > 4:
            ...
            rtf_content += r"{\b " + header_text.replace('\\', '\\\\').replace('{', '\\{').replace('}', '\\}') + r"\par}"
            rtf_content += r"\par"  # Add extra line break after header
            continue
