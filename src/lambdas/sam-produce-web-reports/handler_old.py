...
def process_manual_report_generation() -> Dict[str, Any]:
    ...
    try:
        # Get today's date for report generation with detailed timestamp
...
def generate_dashboard_html(date_display, opportunities_count, matches_count, opportunities, matches, report_type="daily", compact_timestamp=None):
    ...
    if report_type == "manual":
        ...
        page_title = f"{report_title} - {date_display}"
        header_title = f"🚀 {report_title}"
        header_subtitle = f"{date_display} • User-Initiated Analysis"
    else:
        page_title = f"Daily Opportunity Summary - {date_display}"
        header_title = "📊 Daily Opportunity Summary"
        header_subtitle = f"{date_display} • AI-Powered RFP Response Platform"
...
<body class="bg-gray-50">
    ...
    <div class="gradient-bg text-white p-6 mb-8">
        <div class="max-w-6xl mx-auto">
            <h1 class="text-4xl font-bold mb-2">{header_title}</h1>
            <p class="text-xl opacity-90">{header_subtitle}</p>
        </div>
    </div>
